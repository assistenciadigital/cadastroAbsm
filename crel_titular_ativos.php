...
$sql = "SELECT cliente, nome, cpf, datanascimento, classificacao, tipo FROM cliente where status = 'Ativo' and classificacao = 1 ORDER BY nome";
$rs = mysql_query($sql);
?>
<?php
...
$pdf->SetY("-2");
		...
		$pdf-> Cell(0,5,'Socio Efetivo Ativos',0,1);
		$pdf-> Ln(4); #pula 4 linhas
		...
		$contapm    = mysql_num_rows(mysql_query("SELECT tipo from cliente where tipo =2"));
		
		$pdf-> Cell(23,5,'Qtde CBM: '.$contacbm.'',0,1);
		$pdf-> Cell(23,5,'Qtde  PM: '.$contapm.'',0,0);

		
		$pdf-> Output("relatorio_titular_ativos.pdf");
?>  
<script>window.open('relatorio_titular_ativos.pdf'); </script>
</body>
</html>
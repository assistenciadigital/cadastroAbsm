...
if(($_SESSION['login']) AND($_SESSION['nivel'])){
	...
	$sql_conta = mysql_query("SELECT * FROM dependente WHERE titular='".$_GET['id']."'");
	
	$quantreg = mysql_num_rows($sql_conta); // Quantidade de registros pra paginação

	$titular  = @mysql_fetch_assoc(mysql_query("SELECT id, nome FROM cadastro WHERE id='".$_GET['id']."'"));	

	$consulta = mysql_query("SELECT * FROM dependente WHERE titular='".$_GET['id']."' order by nome LIMIT $inicial, $numreg");
...
<table width="800" border="0" align="center" cellpadding="1" cellspacing="1">
  ...
  <div style="color:#009; width:820px; height: 286px; overflow: auto; vertical-align: left;">    
    ...
    <td width="80%" bgcolor="<?php echo $cor?>" ><h6><?php echo $c['nome'].'<br>Nascimento: '.date_data($c['nascimento']); echo ' - Sexo: '.$c['sexo'].' - Parentesco: '.$c['parentesco']?></h6></td>

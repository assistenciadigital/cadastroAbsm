...
$sqlv = "SELECT recibo,valor,emitente,destinatario,referencia,descricao,data,hora,usuario FROM recibo WHERE recibo = '$recibo' and descricao =''ffdescricao";
...
if($contador >= 1) {
  ...
  $sql = "INSERT into recibo(valor,formapagto,doctopagto,banco,agencia,datadocto,emitente,referencia,destinatario,tipo,titular,mes_ano,descricao,data,hora,usuario)
          VALUES('$valor','$formapagto','$doctopagto','$banco','$agencia','$datadocto','$emitente','$referencia','$destinatario','$tipo','$titular','$mes_ano','$descricao','$data_atual','$hora_atual','$loginusuario_atual')";
  $rs = mysql_query($sql);
  $recibo = mysql_insert_id();
     
  #INICIA GRAVACAO DO LOG  
	
  $observacao = ("INC: $data_atual, $hora_atual, Recibo: $recibo, Emitente: $emitente, Destinatario: $destinatario, Tipo: $tipo, Mes/Ano: $mes_ano, Referencia: $referencia, Valor: $valor, Forma/Docto Pagto: $formapagto / $doctopagto, Banco/Agencia: $banco/$agencia, Data Docto: $datadocto, Descricao: $descricao, Usuario: $idusuario_atual - $loginusuario_atual");

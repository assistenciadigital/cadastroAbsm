...
if($contador >= 1) {
  ...
  $sql = "INSERT into medico(cpf,especialidade,conselho,area,limite,nome,cep,endereco,uf,telefone,cidade,bairro,descricao,data,hora,usuario)
          VALUES('$cpf','$especialidade','$conselho','$area','$limite','$nome','$cep','$endereco','$uf','$telefone','$cidade','$bairro','$descricao','$data_atual','$hora_atual','$loginusuario_atual')";
  $rs = mysql_query($sql);
     
  #VERIFICA CODIGO DO REGISTRO INCLUIDO  
  $sqlc = "SELECT medico,cpf,especialidade,conselho,area,limite,nome,cep,endereco,uf,telefone,cidade,bairro,descricao,data,hora,usuario FROM medico WHERE cpf = '$cpf'";
  $rsc = mysql_query($sqlc);

while(list($medicoc,$cpfc,$especialidadec,$conselhoc,$areac,$limitec,$nomec,$cepc,$enderecoc,$ufc,$telefonec,$cidadec,$bairroc,$descricaoc,$datac,$horac,$usuarioc) = mysql_fetch_row($rsc)) {;}
 
    
  #INICIA GRAVACAO DO LOG  
	
  $observacao = ("INC: $data_atual, $hora_atual, Medico: $medicoc, CPF: $cpfc, Especialidade: $especialidadec Conselho: $conselhoc, Área: $areac, Limite: $limitec, Telefone: $telefonec, Endereco: $enderecoc, Bairro: $bairroc, Cidade: $cidadec, UF: $ufc, CEP: $cepc, Obs.: $descricaoc, Usuario: $idusuario_atual - $loginusuario_atual");

...
#data emissao
$data_envia = explode("/",$_POST[fdataemissao]); 
$dia_envia = $data_envia[0];
$mes_envia = $data_envia[1];
$ano_envia = $data_envia[2];
$data_final_envia = "$ano_envia-$mes_envia-$dia_envia";
$data_emissao = $data_final_envia;

#data validade
$data_envia = explode("/",$_POST[fdatavalidade]); 
$dia_envia = $data_envia[0];
$mes_envia = $data_envia[1];
$ano_envia = $data_envia[2];
$data_final_envia = "$ano_envia-$mes_envia-$dia_envia";
$data_validade = $data_final_envia;
...
$nivelusuario_atual = base64_decode($_SESSION['nivel_usuario']);
  ...
  $sql = "INSERT into carteira(codigo_titular,nome_titular,data_nascimento_titular,data_inclusao_titular,codigo_dependente,nome_dependente,data_nascimento_dependente,data_inclusao_dependente,produto_regulamentado,assistencia,acomodacao,dataemissao,via,datavalidade,data,hora,usuario) VALUES('$codigotitular','$nometitular','$data_nascimento_titular','$data_inclusao_titular','$codigodependente','$nomedependente','$data_nascimento_dependente','$data_inclusao_dependente','$produtoregulamentado','$assistencia','$acomodacao','$data_emissao','$via','$data_validade','$data_atual','$hora_atual','$loginusuario_atual')";
  $rs = mysql_query($sql);
...
header("Location:fcar_completa_dependente.php?carteira=$carteirac");
}

if($assistencia == "ODONTOLOGICA") {
header("Location:fcar_odontologica_dependente.php?carteira=$carteirac");
}

if($assistencia == "MEDICA") {
header("Location:fcar_medico_dependente.php?carteira=$carteirac");
}
}
?>
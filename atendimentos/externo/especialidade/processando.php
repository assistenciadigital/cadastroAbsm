...
//------------------------------------->> DELETAR
	...
	if(base64_decode($_SESSION['nivel_usuario'])<=2){
			...
			$result_log = mysql_query("select * from convenio_especialidade where id='".$_GET['id']."'");
			if (!$result_log) {
			...
			if (mysql_num_rows($result_log) > 0) {
				while ($row = mysql_fetch_assoc($result_log)) {
						$observacao = ($_GET['acao'].', Date: '.date('Y-m-d H:i:00').', User: '.$_SESSION['login_usuario'].', ID: '.$row[id].', Especialidade: '.$row[especialidade].', Nome:'.$row[nome].', Descricao: '.$row[descricao].', Data/Hora: '.$row[data].', Usuario: '.$row[usuario]);
					}
				}	
			...
			mysql_query ("DELETE FROM convenio_especialidade WHERE id='".$_GET['id']."'");
...
//------------------------------------->> ALTERAR	
	...
	if(base64_decode($_SESSION['nivel_usuario'])<=2){
		...
		// inicio guarda informacao para o log
		$result_log = mysql_query("select * from convenio_especialidade where id='".$_POST['id']."'");
		if (!$result_log) {
			echo 'Could not run query: ' . mysql_error();
			exit;
		}
	...
	// fim guarda informacao para o log	
			...
			$altera = mysql_query("UPDATE convenio_especialidade SET especialidade='".$_POST['especialidade']."', nome='".trim(addslashes(htmlentities(strtoupper($_POST['nome']))))."', descricao='".trim(addslashes(htmlentities(strtoupper($_POST['descricao']))))."', data='".date('Y-m-d H:i:00')."', usuario='".$_SESSION['login_usuario']."' where id='".$_POST['id']."'");
...
//------------------------------------->> INCLUIR
		...
		if (!(mysql_num_rows (mysql_query("SELECT id FROM convenio_especialidade WHERE especialidade='".$_POST['especialidade']."' AND nome='".$_POST['nome']."'")))) {
					
			$insere = mysql_query ("INSERT convenio_especialidade(especialidade, nome, descricao, data, usuario) VALUES ('".$_POST['especialidade']."', '".trim(addslashes(htmlentities(strtoupper($_POST['nome']))))."', '".trim(addslashes(htmlentities(strtoupper($_POST['descricao']))))."', '".date('Y-m-d H:i:00')."', '".$_SESSION['login_usuario']."')");
		$id = mysql_insert_id();
			
		// inicio guarda informacao para o log
		$result_log = mysql_query("select * from convenio_especialidade where id='".mysql_insert_id()."'");
		if (!$result_log) {
			echo 'Could not run query: ' . mysql_error();
			exit;
		}
		if (mysql_num_rows($result_log) > 0) {
			while ($row = mysql_fetch_assoc($result_log)) {
				$observacao = ('inclusao, Date: '.date('Y-m-d H:i:00').', User: '.$_SESSION['login_usuario'].', ID: '.$row[id].', Especialidade: '.$row[especialidade].', Nome: '.$row[nome].', Descricao: '.$row[descricao].', Data/Hora: '.$row[data].', Usuario: '.$row[usuario]);
			}
		}	

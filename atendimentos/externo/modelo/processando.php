...
//------------------------------------->> DELETAR
	...
	if(base64_decode($_SESSION['nivel_usuario'])<=2){
			...
			$result_log = mysql_query("select * from convenio where id='".$_GET['id']."'");
			if (!$result_log) {
			...
			if (mysql_num_rows($result_log) > 0) {
				while ($row = mysql_fetch_assoc($result_log)) {
						$observacao = ($_GET['acao'].', Date: '.date('Y-m-d H:i:00').', User: '.$_SESSION['login_usuario'].', ID: '.$row[id].', Inscricao: '.$row[inscricao].', Razao:'.$row[razao].', Fantasia: '.$row[fantasia].', Endereco: '.$row[endereco].', Numero: '.$row[numero].', Complemento: '.$row[complemento].', Bairro: '.$row[bairro].', Cidade: '.$row[cidade].', UF: '.$row[uf].', CEP: '.$row[cep].', Fone: '.$row[fone].', Fax: '.$row[fax].', E-mail: '.$row[email].', Autorizador: '.$row[autorizador].', Funcao: '.$row[funcao].', Setor: '.$row[setor].', Data/Hora: '.$row[data].', Usuario: '.$row[usuario]);
					}
				}	
			...
			mysql_query ("DELETE FROM convenio WHERE id='".$_GET['id']."'");
...
//------------------------------------->> ALTERAR	
	...
	if(base64_decode($_SESSION['nivel_usuario'])<=2){
		...
		// inicio guarda informacao para o log
		$result_log = mysql_query("select * from convenio where id='".$_POST['id']."'");
		if (!$result_log) {
			echo 'Could not run query: ' . mysql_error();
			exit;
		}
	...
	// fim guarda informacao para o log	
			...
			$altera = mysql_query("UPDATE convenio SET inscricao='".$_POST['inscricao']."', razao='".trim(addslashes(htmlentities(strtoupper($_POST['razao']))))."', fantasia='".trim(addslashes(htmlentities(strtoupper($_POST['fantasia']))))."', cep='".$_POST['cep']."', endereco='".trim(addslashes(htmlentities(strtoupper($_POST['endereco']))))."', numero='".$_POST['numero']."', complemento='".trim(addslashes(htmlentities(strtoupper($_POST['complemento']))))."', bairro='".trim(addslashes(htmlentities(strtoupper($_POST['bairro']))))."', cidade='".trim(addslashes(htmlentities(strtoupper($_POST['cidade']))))."', uf='".trim(addslashes(htmlentities(strtoupper($_POST['uf']))))."', fone='".$_POST['fone']."', fax='".$_POST['fax']."', email='".trim(addslashes(htmlentities(strtolower($_POST['email']))))."', autorizador='".trim(addslashes(htmlentities(strtolower($_POST['autorizador']))))."', funcao='".trim(addslashes(htmlentities(strtolower($_POST['funcao']))))."', setor='".trim(addslashes(htmlentities(strtolower($_POST['setor']))))."', data='".date('Y-m-d H:i:00')."', usuario='".$_SESSION['login_usuario']."' where id='".$_POST['id']."'");
...
//------------------------------------->> INCLUIR
		...
		if (mysql_num_rows($result_log) > 0) {
			while ($row = mysql_fetch_assoc($result_log)) {
				$observacao = ('inclusao, Date: '.date('Y-m-d H:i:00').', User: '.$_SESSION['login_usuario'].', ID: '.$row[id].', Inscricao: '.$row[inscricao].', Razao:'.$row[razao].', Fantasia: '.$row[fantasia].', Endereco: '.$row[endereco].', Numero: '.$row[numero].', Complemento: '.$row[complemento].', Bairro: '.$row[bairro].', Cidade: '.$row[cidade].', UF: '.$row[uf].', CEP: '.$row[cep].', Fone: '.$row[fone].', Fax: '.$row[fax].', E-mail: '.$row[email].', Autorizador: '.$row[autorizador].', Funcao: '.$row[funcao].', Setor: '.$row[setor].', Data/Hora: '.$row[data].', Usuario: '.$row[usuario]);
			}
		}	

...
//------------------------------------->> DELETAR
	...
	if(base64_decode($_SESSION['nivel_usuario'])<=2){
			...
			$result_log = mysql_query("select * from convenio_empresa where id='".$_GET['id']."'");
			if (!$result_log) {
...
//------------------------------------->> ALTERAR	
	...
	if(base64_decode($_SESSION['nivel_usuario'])<=2){
		...
		// inicio guarda informacao para o log
		$result_log = mysql_query("select * from convenio_empresa where id='".$_POST['id']."'");
		if (!$result_log) {
			echo 'Could not run query: ' . mysql_error();
			exit;
		}
...
//------------------------------------->> INCLUIR
		...
		if (!(mysql_num_rows (mysql_query ("SELECT id FROM convenio_empresa WHERE inscricao='".$_POST['inscricao']."'")))) {		
		...
		// inicio guarda informacao para o log
		$result_log = mysql_query("select * from convenio_empresa where id='".mysql_insert_id()."'");
		if (!$result_log) {
			echo 'Could not run query: ' . mysql_error();
			exit;
		}
